...
def cubic_coeffs(x, y):
    coeffs = np.polyfit(x, y, 3)  # Cubic fit (degree 3)
    return coeffs
...
# combined cubic regression 
def combined_cubic_coeffs(x1, x2, y):
...
# cubic coefficients 
# individual analyses
coeffs_r_la = cubic_coeffs(x_l, y_l)
coeffs_r_exp = cubic_coeffs(x_l, y_exp)
coeffs_t_la = cubic_coeffs(x_t, y_l)
coeffs_t_exp = cubic_coeffs(x_t, y_exp)
...
# Combined cubic regression 
# Limit Analysis
model_la, poly_la, scaler_la = combined_cubic_coeffs(x_l, x_t, y_l)
...
# Experimental
model_exp, poly_exp, scaler_exp = combined_cubic_coeffs(x_l, x_t, y_exp)
...
print("\nCubic Regression Equations:")
print(f"r/L vs Limit Analysis: y = {coeffs_r_la[0]:.4e}x^3 + {coeffs_r_la[1]:.4e}x^2 + {coeffs_r_la[2]:.4e}x + {coeffs_r_la[3]:.4e}")
print(f"r/L vs Experimental: y = {coeffs_r_exp[0]:.4e}x^3 + {coeffs_r_exp[1]:.4e}x^2 + {coeffs_r_exp[2]:.4e}x + {coeffs_r_exp[3]:.4e}")
print(f"t/L vs Limit Analysis: y = {coeffs_t_la[0]:.4e}x^3 + {coeffs_t_la[1]:.4e}x^2 + {coeffs_t_la[2]:.4e}x + {coeffs_t_la[3]:.4e}")
print(f"t/L vs Experimental: y = {coeffs_t_exp[0]:.4e}x^3 + {coeffs_t_exp[1]:.4e}x^2 + {coeffs_t_exp[2]:.4e}x + {coeffs_t_exp[3]:.4e}")

# Combined cubic regression equation 
# Limit Analysis
coefs_combined_la = model_la.coef_
intercept_combined_la = model_la.intercept_
print("\nCombined Limit Analysis Equation:")
print(f"y = {intercept_combined_la:.4e} + {coefs_combined_la[1]:.4e}*x1 + {coefs_combined_la[2]:.4e}*x2 + {coefs_combined_la[3]:.4e}*x1^2 + {coefs_combined_la[4]:.4e}*x1*x2 + {coefs_combined_la[5]:.4e}*x2^2 + {coefs_combined_la[6]:.4e}*x1^3 + {coefs_combined_la[7]:.4e}*x1^2*x2 + {coefs_combined_la[8]:.4e}*x1*x2^2 + {coefs_combined_la[9]:.4e}*x2^3")

# Experimental
coefs_combined_exp = model_exp.coef_
intercept_combined_exp = model_exp.intercept_
print("\nCombined Experimental Equation:")
print(f"y = {intercept_combined_exp:.4e} + {coefs_combined_exp[1]:.4e}*x1 + {coefs_combined_exp[2]:.4e}*x2 + {coefs_combined_exp[3]:.4e}*x1^2 + {coefs_combined_exp[4]:.4e}*x1*x2 + {coefs_combined_exp[5]:.4e}*x2^2 + {coefs_combined_exp[6]:.4e}*x1^3 + {coefs_combined_exp[7]:.4e}*x1^2*x2 + {coefs_combined_exp[8]:.4e}*x1*x2^2 + {coefs_combined_exp[9]:.4e}*x2^3")
...
plt.plot(x_vals_r_la, np.polyval(coeffs_r_la, x_vals_r_la), color='red', label='Limit Analysis Cubic')
plt.xlabel('r/L (m)')
...
plt.plot(x_vals_r_exp, np.polyval(coeffs_r_exp, x_vals_r_exp), color='orange', label='Experimental Cubic')
plt.xlabel('r/L (m)')
...
plt.plot(x_vals_t_la, np.polyval(coeffs_t_la, x_vals_t_la), color='cyan', label='Limit Analysis Cubic')
plt.xlabel('t/L (m)')
plt.ylabel('Limit Analysis (kN)')
...
plt.plot(x_vals_t_exp, np.polyval(coeffs_t_exp, x_vals_t_exp), color='yellow', label='Experimental Cubic')
plt.xlabel('t/L (m)')
plt.ylabel('Experimental (kN)')
